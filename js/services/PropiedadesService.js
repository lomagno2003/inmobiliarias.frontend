...
myApp.service('PropiedadesService', function($resource) {	
    this.loadRepository = function(fnc){
    	propiedadesAPI = $resource("http://localhost:port/propiedades", {port:':8082'});
    	
		var returnValue = propiedadesAPI.get(function(){
			if(returnValue.hasOwnProperty('_embedded')){
				fnc(returnValue._embedded.propiedades);
			} else {
				fnc({});
			}
		});
    ...
    this.loadElement = function(id,fnc){
    	propiedadesAPI = $resource("http://localhost:port/propiedades/:idPropiedad", {port:':8082', idPropiedad:id});
